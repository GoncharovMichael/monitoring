...
do
	if [[ $coutfile -eq 101 ]]; then
		...
		echo "Завершение-$(date +"%D-%T"). Время выполнения скрипта составляет $datetotal секунд." >> 02.log
		echo "Cтарт-$datestartscript. Завершение-$(date +"%D-%T"). Время выполнения сценария составляет $datetotal секунд." 
		exit 0
		fi
	if [[ $v -eq $len1 ]]; then
		v=0
...
dateend=$(date +%s)
datetotal=$((dateend-datestart))
echo "Завершение-$(date +"%D-%T"). Время выполнения сценария составляет $datetotal секунд." >> 02.log
echo "Старт-$datestartscript. Завершение-$(date +"%D-%T"). Время выполнения сценария составляет $datetotal секунд." 