...
do
	...
	date1=$(date +%d%m%y)
	log_date1=$(date +%F)
	mkdir "$url"/"$abcdir""$word1"_"$date1" >> error.log
	...
	do
		...
		date2=$(date +%d%m%y)
		log_date2=$(date +%F)
		touch "$url"/"$abcdir""$word1"_"$date1"/"$filename""$word2"_"$date2"."$extension" >> error.log
		fallocate -l "$filesize"K "$url"/"$abcdir""$word1"_"$date1"/"$filename""$word2"_"$date2"."$extension" >> error.log
